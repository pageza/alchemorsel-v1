...
func (s *SecurityTestSuite) CheckSSLConfiguration(t *testing.T, url string) {
	...
	assert.GreaterOrEqual(t, state.Version, tls.VersionTLS12, "Should use TLS 1.2 or higher")
}
...
func TestSecurity_Headers(t *testing.T) {
	...
	suite.CheckSecurityHeaders(t, "https://example.com")
}
...
func TestSecurity_SSL(t *testing.T) {
	logger := zap.NewNop()
	suite := NewSecurityTestSuite(logger)
	suite.CheckSSLConfiguration(t, "example.com:443")
}
...
func TestSecurity_SQLInjection(t *testing.T) {
	logger := zap.NewNop()
	suite := NewSecurityTestSuite(logger)
	suite.CheckSQLInjection(t, "https://example.com/api/users")
}
...
func TestSecurity_XSS(t *testing.T) {
	logger := zap.NewNop()
	suite := NewSecurityTestSuite(logger)
	suite.CheckXSSVulnerabilities(t, "https://example.com/api/search")
}
...
func TestSecurity_Authentication(t *testing.T) {
	...
	suite.CheckSecurityHeaders(t, "https://example.com/auth")
