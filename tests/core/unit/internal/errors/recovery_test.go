...
func TestRecoveryStrategies(t *testing.T) {
	// Test retry strategy
	retryStrategy := errors.RetryStrategy(3, time.Millisecond)
	err := errors.New("RETRY_ERROR", "retry error")
	recoveredErr := retryStrategy(context.Background(), err)
	assert.NoError(t, recoveredErr)

	// Test fallback strategy
	fallbackCalled := false
	fallbackStrategy := errors.FallbackStrategy(func() error {
		fallbackCalled = true
		return nil
	})
	err = errors.New("FALLBACK_ERROR", "fallback error")
	recoveredErr = fallbackStrategy(context.Background(), err)
	assert.NoError(t, recoveredErr)
	assert.True(t, fallbackCalled)

	// Test circuit breaker strategy
	circuitBreaker := errors.CircuitBreakerStrategy(3, time.Second)
	err = errors.New("CIRCUIT_ERROR", "circuit error")
	for i := 0; i < 4; i++ {
	...
	timeoutStrategy := errors.TimeoutStrategy(time.Millisecond)
	err = errors.New("TIMEOUT_ERROR", "timeout error")
...
func TestErrorAnalytics(t *testing.T) {
	...
	err := errors.New("TEST_ERROR", "test error")
	for i := 0; i < 5; i++ {
		tracker.TrackError(err)
	}

	// Register recovery strategy and recover some errors
	tracker.RegisterRecoveryStrategy("TEST_ERROR", errors.RetryStrategy(3, time.Millisecond))
	for i := 0; i < 3; i++ {
		tracker.TrackError(err)
	}
...
func TestErrorLatencyTracking(t *testing.T) {
	...
	err := errors.New("LATENCY_ERROR", "latency error")
	start := time.Now()
	tracker.TrackError(err)
